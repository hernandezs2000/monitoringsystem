...
/* ------------AYOS NG FORMAT OF DATE------------------- */
/* $datec = count($date);
$rlidcount = $datec - 1;
$year = $month = $day = array();
    for($ctr0 = 0; $ctr0 <= $datec; $ctr0++){
        $year0 = substr($date[$ctr0],0,4);
        $month0 = substr($date[$ctr0],4,6);
        $day0 = substr($date[$ctr0],6);
        $year[] = $year0;
        $month[] = $month0;
        $day[] = $day0;  
    } */
       /* ------------AYOS NG FORMAT OF TIME-------------------  22.49*/